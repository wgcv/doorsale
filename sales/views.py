    ...
    def post(self, request):
        ...
        try:
            ...
            if payment_method == PaymentMethod.CREDIT_CARD:
                return HttpResponseRedirect(reverse('payments_process_online', kwargs={"order_id":order.id, "receipt_code":order.receipt_code}))
            else:
...
class CheckoutReceiptView(CheckoutBaseView):
    ...
    def get(self, request, order_id, receipt_code):
        ...
        if order_confirmed:
            ...
            context = Context(
                {'order': order, 'user': request.user, 'DOMAIN': "localhost:8000",
            'SITE_NAME': 'Doorsale'})
